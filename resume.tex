...
{Remote}
{
	...
	{June 2023 -- Present}
	{
		\begin{itemize}[nolistsep]
			\item Designed and shipped new user-facing features providing auto billing for 50K\texttt{+} tasks that were manually billed
			\item Created custom invoicing flow using SQS + Lambdas + Chargebee API to generate invoices
			\item Fixed issues in pricing and billing overviews of client-facing React frontend
			\item Designed launch readiness dashboard using Datadog metrics to monitor traffic to new features
			\item Designed performant queries for MySQL database using knex, on tables with 15M\texttt{+} rows
	...
	{May 2022 -- June 2023}
	{
		\begin{itemize}[nolistsep]
			...
			\item Integrated payment systems like Stripe, Plaid, Braintree servicing 10K\texttt{+} customers, collecting 100M\texttt{+} in revenue

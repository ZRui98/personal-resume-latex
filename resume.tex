...
{Remote}
{
	...
	{Feb 2024 -- Present}
	{
		\begin{itemize}[nolistsep]
			\item Implemented Multi-account support for 5\texttt{+} new accounts on internal tool used to deploy code to AWS EC2 instances
			\item Designed and built features in Spinnaker CD like Kubernetes deploys, Buildkite CI, and timed rollbacks, increasing adoption by 10\%
			\item Decreased cpu/memory resource usage by 30\% by optimizing how internal deployment logic works for Spinnaker pipelines
			\item Improved performance of some MySQL queries by up to 20\% by removing nested joins and adding indices on commonly used query fields
			\item Updated GraphQL endpoints and React UI to introduce new types to Spinnaker enums
			\item Designed shadow deployment flow for kubernetes migrations, which was critical to migrating 50\texttt{+} services to onboard onto kubernetes
			\item Mentored new teammates by onboarding them onto codebase, and improved internal tooling by adding hot reloading for PF4J plugins
		\end{itemize}
	}
}

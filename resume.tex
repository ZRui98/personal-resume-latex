...
{Remote}
{
	...
	{Feb 2024 -- Present}
	{
		\begin{itemize}[nolistsep]
			\item Implemented Multi-account support for 3\texttt{+} new accounts on internal service used to deploy code to AWS EC2
			\item Built dedicated developer build workflow, which sped up development speed by 300\%
			\item Designed and built features in Spinnaker CD (Kubernetes deploys, Buildkite CI, and timed rollbacks), increasing adoption by 15\texttt{+}\%
			\item Reduced cpu/memory resource usage by 30\% by optimizing internal logic for Spinnaker constraint evaluations
			\item Improved MySQL performance by up to 20\% through query analysis and optimization
			\item Enhanced developer and user experience of GraphQL backend and React UI
			\item Architected a Kubernetes migration deployment flow, enabling the successful migration of 50+ services from EC2 to Kubernetes
			\item Formally mentored new teammates by onboarding them onto kotlin/java/python codebase
			\item Led improvements of internal tooling by adding live reloading for PF4J plugins
		\end{itemize}
	}
}
